...
- **`create_auth_session` / `createAuthSession`** — calls `POST /v1/auth/sessions` with `email` and `password`; returns the issued access token payload.
- **`refresh_auth_session` / `refreshAuthSession`** — calls `POST /v1/auth/sessions/refresh` using the CSRF token + refresh cookie session.
- **`validate_access_token` / `validateAccessToken`** — validates token by calling `GET /v1/me`.
